...
def air_traffic_controller(jsoninput):
    ...
    for flight in flights:
        ...
        flight["int_time"] = 60 * \
            int(flight["Time"][:2]) + int(flight["Time"][2:])
        del flight["Time"]
    ...
    for flight in flights:
        flight["superstring"] = flight["str_time"] + \
            flight["PlaneId"] + flight["d"]
    ...
    distressed_flights = sorted(
        distressed_flights, key=lambda k: k['superstring'])
    # print(distressed_flights)
    ...
    for j, flight in enumerate(flights):
        ...
        if c < len(distressed_flights):
            ...
            while loop_lock:  # in case there are consecutive distressed flights
                ...
                if distressed_flights[c]["int_time"] - reserve_time < min(runways_avail):
                    ...
                    # reserve the time
                    runways_avail[i] = distressed_flights[c]["int_time"] + \
                        reserve_time
        ...
        # in case it ends with distressed flights
        if j == len(flights) - 1 and c < len(distressed_flights):
            loop_lock = True
            while loop_lock:  # in case there are consecutive distressed flights
                # print("===")
                #print(distressed_flights[c]["int_time"], min(runways_avail), j)
    ...
    for flight in flights:
        flight['Time'] = format(flight["arr_time"] // 60,
                                '02') + format(flight["arr_time"] % 60, '02')
